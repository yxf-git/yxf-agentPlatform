...
import { Plus, Upload, Sparkles, Zap, Trash2, Loader2, FolderOpen } from "lucide-vue-next"
import { useSkillStore } from "@/stores/skills"
import { useChatStore } from "@/stores/chat"
import { fetchSkillsDir, openSkillsDir, type ApiSkill } from "@/lib/api"
...
onMounted(async () => {
  ...
  try {
    skillsPath.value = await fetchSkillsDir()
  } catch {
    skillsPath.value = ""
  }
})
...
async function generate() {
  ...
  if (skill) {
    ...
    createError.value = "生成失败，请在设置中配置模型 API Key"
  }
}
...
<template>
  <div class="flex h-full flex-col">
    ...
    <p
      v-if="skillsPath"
      class="truncate px-4 pb-2 text-[11px] text-stone-400"
      :title="skillsPath"
    >
      存放：{{ skillsPath }}
    </p>
    ...
    <div class="flex-1 overflow-y-auto px-3 pb-4">
      ...
      >
        暂无技能。创建/上传/市场导入都会保存到上方技能目录；对话里生成的文档则写入工作区。
      </p>
    </div>
  </div>
</template>
